...
import { StudentMetricCard, InsightBlock } from "@/components/user/dashboard/ui-cards";
  ...
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center bg-black">
        <div className="animate-pulse text-zinc-500 font-mono text-sm uppercase tracking-widest">
          INITIALIZING_SYSTEM...
        </div>
      </div>
    );
    ...
    <main className="relative z-10 flex-1 px-8 py-4 lg:px-16">
      <div className="mb-12 flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-zinc-500 max-w-md">
            Your personalized AI-driven academic command center is ready.
          </p>
        </div>
      ...
      <footer className="mt-20 flex items-center justify-between border-t border-white/5 pt-10 opacity-60">
        <div className="flex items-center gap-4 group cursor-default">
          <span className="text-[11px] font-black text-white tracking-[0.2em] uppercase group-hover:text-blue-400 transition-colors">
            ExamAI
          </span>
          <span className="text-[10px] font-mono text-zinc-500 italic">
            Core v2.0 | Session: {new Date().toLocaleDateString()}
          </span>
        </div>

        <Badge className="bg-zinc-800 text-zinc-400 border-none px-3 py-1 font-mono text-[9px]">
          LIVE_SYNC_OK
        </Badge>
      </footer>
    </main>
